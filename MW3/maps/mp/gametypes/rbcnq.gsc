...
endRound(roundwinner)
{
	...
	if(level.ex_swapteams == 1 && !isdefined(level.rbcnq_swap_roundwinner)) extreme\_ex_main::swapTeams();
	if(level.ex_swapteams == 2 && !isdefined(level.rbcnq_swap_roundwinner) && game["roundnumber"] == level.half_time) extreme\_ex_main::swapTeams();
...
updateGametypeCvars()
{
	for(;;)
	{
		...
		scorelimit = getcvarint("scr_rbchq_scorelimit");
			...
			{	
				if(self.script_idnumber == allSpawnObjectives[n].script_idnumber)
				{
					if(other.pers["team"] == game["attackers"])
					{
						...
						{
							while(isAlive(other) && other isOnGround() && other useButtonPressed())
							{
								...
								other.progressbackground setShader("black", (192 + 4), 12);

								if(!isdefined(other.progressbar))
								{
									other.progressbar = newClientHudElem(other);
									other.progressbar.x = int(192 / (-2.0));
								...
								other.progressbar scaleOverTime(level.captime, 192, 8);

								other playsound("MP_bomb_plant");
								other linkto(self); 	// Stop Player moving while capping
								other [[level.ex_dWeapon]](); // No guns while capping
								other.progresstime = 0; // Time player has been capping

								while(isAlive(other) && other useButtonPressed() && (other.progresstime < level.captime)) // Change Variables
								{
									other.progresstime += 0.05;
									wait( [[level.ex_fpstime]](0.05) );
								}

								other.progressbackground destroy(); // Destroy Bar Background
								other.progressbar destroy(); // Destroy Bar
								other [[level.ex_eWeapon]](); // Enable Players Weapon
								other unlink(); // Unlink Player
								self.beingCapped = undefined; // Nobody is capping objective
					...
					{ 
						...
						if((n == allSpawnObjectives.size - 1) || (isdefined(previousObjective) && isOff(previousObjective)))
						{
							while(other isTouching(self.trigger) && isAlive(other) && other isOnGround() && other useButtonPressed())
							{
								...
								while(isAlive(other) && other useButtonPressed() && (other.progresstime < level.captime))
								{
									other.progresstime += 0.05;
									wait( [[level.ex_fpstime]](0.05) );
								}
								...
								other [[level.ex_eWeapon]](); // Enable Players Weapon
								other unlink(); // Unlink Player
								self.beingCapped = undefined; // Nobody is capping
				...
				{ 
					if(self.isAvailable == 1) // and it's not in a wait state
					{
						while(isAlive(other) && other isOnGround() && other useButtonPressed())
						{
							...
							other [[level.ex_eWeapon]](); // Enable Players Weapon
							other unlink(); // Unlink Player
							other.beingCapped = undefined; // Nobody capping
...
awardPoints(player, objectiveType) 
{
	...
	player.pers["score"] += playerPoints;
