...
startCaptureProgress(clientid, team)
{
	...
	if(self.team != "none") 
	{
		...
		while(isDefined(self) && time < time_neutral) 
		{
			...
			// Abort
			if(!isDefined(origin) || (isDefined(origin) && distance(origin, self.origin) > level.flag_radius) || checkOtherPlayerInRange(clientid, origin, EnemyTeam(team), level.flag_radius) > -1)
			{
				level changeLevelHudElementByName("flag_" + self.objective,0.8);
	...
	level createLevelHudElement("flag_" + self.objective, 325 + 36 * self.objective - 18 * (level.flags.size - 1), level.hud_dom_pos_y, "center", "middle", "fullscreen", "fullscreen", false, level.hudicon_empty, 32, 32, 1, 0.8, 1, 1, 1);
...
GetFlag(flag) 
{
	...
	if(self.pers["team"] == "allies") 
	{
		...
		level createLevelHudElement("flag_" + flag.objective, 325 + 36 * flag.objective - 18 * (level.flags.size - 1), level.hud_dom_pos_y, "center", "middle", "fullscreen", "fullscreen", false, game["hudicon_allies"], 32, 32, 1, 0.8, 1, 1, 1);
	}
	else 
	{
		flag.team = "axis";
		...
		level deleteLevelHudElementByName("flag_" + flag.objective);
		level createLevelHudElement("flag_" + flag.objective, 325 + 36 * flag.objective - 18 * (level.flags.size - 1), level.hud_dom_pos_y, "center", "middle", "fullscreen", "fullscreen", false, game["hudicon_axis"], 32, 32, 1, 0.8, 1, 1, 1);
	}
