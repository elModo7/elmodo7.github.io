...
Callback_StartGameType()
{
	...
	level.zradioradius = level.ex_hq_radio_zradius;
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if(level.roundStarted && level.respawndelay && !allowInstantRespawn)
	{
		...
		if(isdefined(players[i].pers["team"]) && players[i].pers["team"] != "spectator" && players[i].sessionstate == "playing")
		{
			...
			if(isdefined(players[i].isscorer))
			{
				if(!level.ex_hqpoints_radius || (distance(players[i].origin, radio.origin) <= level.ex_hqpoints_radius))
				{
					...
					{
						players[i].score += level.ex_hqpoints_playercap;
						if(level.ex_ranksystem) players[i].pers["special"]+= level.ex_hqpoints_playercap;
						// added for arcade style HUD points
					...
					{
						players[i].score += level.ex_hqpoints_playerneut;
						if(level.ex_ranksystem) players[i].pers["special"]+= level.ex_hqpoints_playerneut;
						// added for arcade style HUD points
						players[i] notify("update_playerscore_hud");
	...
	else
	{
		...
		if(isdefined(level.DefendingRadioTeam))
		{
			if((level.DefendingRadioTeam == "allies") || (level.DefendingRadioTeam == "axis"))
			{
				...
				{
					setTeamScore(level.DefendingRadioTeam, getTeamScore(level.DefendingRadioTeam) + level.ex_hqpoints_teamcap);
					level notify("update_teamscore_hud");
				}
			}
		}
...
hq_points()
{
	while(!level.mapended)
	{
		if(level.DefendingRadioTeam != "none")
		{
			if(getTeamCount(level.DefendingRadioTeam))
			{
				setTeamScore(level.DefendingRadioTeam, getTeamScore(level.DefendingRadioTeam) + level.ex_hqpoints_defpps);
				level notify("update_teamscore_hud");
				checkScoreLimit();
			}
		}
...
updateTimer()
{
	if(isdefined(self.pers["team"]) && (self.pers["team"] == "allies" || self.pers["team"] == "axis") && isdefined(self.pers["weapon"]))
	{
		...
		{
			if(isdefined(self.respawntimer)) self.respawntimer.alpha = 0;
			if(isdefined(self.staydead)) self.staydead.alpha = 1;
		}
		else
		{
			if(isdefined(self.respawntimer)) self.respawntimer.alpha = 1;
			if(isdefined(self.staydead)) self.staydead.alpha = 0;
		}
	}
	else
	{
