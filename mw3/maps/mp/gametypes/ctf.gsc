...
spawnPlayer()
{
	...
	if(level.scorelimit > 0) self setClientCvar("cg_objectiveText", &"MP_CTF_OBJ_TEXT", level.scorelimit);
		...
		if(isPlayer(other) && isAlive(other) && (other.pers["team"] != "spectator"))
		{
			if(other.pers["team"] == self.team) // Touched by team
			{
				if(self.atbase)
				{
					if(isdefined(other.flag)) // Captured flag
					{
						...
						if(level.ex_ranksystem) other.pers["special"] += level.ex_ctfpoints_playercf;
						// added for arcade style HUD points
				...
				else // Returned flag
				{
					...
					other.pers["flagret"]++;
					other.score += level.ex_ctfpoints_playerrf;
					if(level.ex_ranksystem) other.pers["special"] += level.ex_ctfpoints_playerrf;
					other notify("update_playerscore_hud");
			...
			else if(other.pers["team"] != self.team) // Touched by enemy
			{
				...
				if(self.atbase) // Stolen flag
				{
					...
					other.score += level.ex_ctfpoints_playersf;
					if(level.ex_ranksystem) other.pers["special"] += level.ex_ctfpoints_playersf;
					other notify("update_playerscore_hud");
				...
				else // Picked up flag
				{
					...
					{
						other.score += level.ex_ctfpoints_playertf;
						if(level.ex_ranksystem) other.pers["special"] += level.ex_ctfpoints_playertf;
						other notify("update_playerscore_hud");
					}
